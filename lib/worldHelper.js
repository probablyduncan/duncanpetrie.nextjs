...
/**
 * if there is a filter, return table of contents and all filter categories
 * otherwise, return table of contents and featured groups
 * 
 * @returns 
 */
export function assembleCardGroups(cardData, filter, filterText) {
    const groupsFlatMap = getGroupsFlatMap(cardData);

    return filter ? getFilterGroups(cardData, groupsFlatMap, filter, filterText) : Object.entries(groupBy(groupsFlatMap.filter(g => g.featuredGroup), 'group'));
}
...
/**
 * takes a world card frontmatter
 * 
...
export const capitalize = (str) => str.replace(/(\b[a-z](?!\s))/g, char => char.toUpperCase())